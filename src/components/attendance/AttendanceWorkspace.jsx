...
const fullName = (person) => [person.apellido_paterno, person.apellido_materno, person.nombres].filter(Boolean).join(" ");
const initials = (student) => [student.apellido_paterno, student.apellido_materno, student.nombres].filter(Boolean).slice(0, 2).map((part) => part.trim().charAt(0)).join("").toUpperCase();
const compareStudents = (left, right) => fullName(left).localeCompare(fullName(right), "es", { sensitivity: "base" });
...
export default function AttendanceWorkspace({ selection }) {
  ...
  const [responsibleTeacher, setResponsibleTeacher] = useState(null);
  ...
  useEffect(() => {
    const controller = new AbortController();
    setStudents([]); setAttendance({}); setSavedAttendance({}); setResponsibleTeacher(null); setError(""); setFeedback("");
    ...
    getMatriculas({ id_seccion: sectionId, estado: true, signal: controller.signal })
      .then(async (enrollments) => {
        ...
        const sectionProgram = sectionPrograms.find((item) => String(item.id_seccion) === String(sectionId));
        const teacher = sectionProgram?.id_docente_responsable
  ...
  return <section className="attendance-workspace">
    ...
    {filtersComplete && !loading && !error && <p className="attendance-responsible-teacher"><span>Docente responsable de la sección:</span> <strong>{responsibleTeacher ? fullName(responsibleTeacher) : "Sin docente asignado"}</strong></p>}
