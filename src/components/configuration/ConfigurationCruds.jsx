...
export function ProgramacionSeccionesCrud({ onEdit }) {
  ...
  return <CrudPanel initial={initial} load={loadFiltered} idOf={(x) => x.row_key} noun="Registro de sección" canOperate={Boolean(academic.grado)} showActions={false} autoShowResults resultsTrigger={academic.grado} onEditItem={onEdit}
    ...
    columns={[{ label: "Sección", value: (x) => x.seccion?.nombre }, { label: "Docente responsable", value: (x) => personName(x.docente_responsable || x.docente || {}) || (docentes.find((item) => text(item.id_docente) === text(x.id_docente_responsable)) ? personName(docentes.find((item) => text(item.id_docente) === text(x.id_docente_responsable))) : "Sin asignar") }, { label: "Estado", value: (x) => x.id_programacion_seccion ? (booleanValue(x.estado) ? "Activo" : "Inactivo") : "Sin registro" }]}
    validate={(f) => !f.id_seccion || !f.id_docente_responsable ? "Selecciona la sección y el docente responsable." : ""} save={(f) => f.id ? updateProgramacionSeccion(f.id, f.id_seccion, f.id_docente_responsable, f.estado === "true") : createProgramacionSeccion(f.id_seccion, f.id_docente_responsable)} editValues={(x) => ({ ...initial, id: x.id_programacion_seccion, id_seccion: text(x.id_seccion), id_docente_responsable: text(x.id_docente_responsable), estado: String(booleanValue(x.estado)) })} renderFields={(f, c) => <><Field label="Sección" name="id_seccion" value={f.id_seccion} onChange={c}>{academic.secciones.map((x) => <option key={x.id_seccion} value={x.id_seccion}>{x.nombre}</option>)}</Field><Field label="Docente responsable" name="id_docente_responsable" value={f.id_docente_responsable} onChange={c}>{docentes.map((x) => <option key={x.id_docente} value={x.id_docente}>{personName(x)}</option>)}</Field>{f.id && <StateField value={f.estado} onChange={c} />}</>} />;
}
